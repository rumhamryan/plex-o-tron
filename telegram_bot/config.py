...
def get_configuration() -> Tuple[str, Dict[str, str], List[int], Dict[str, str], Dict[str, Any]]:
    ...
    if not os.path.exists(config_path):
        logger.critical(f"Configuration file '{config_path}' not found. Please create it.")
        sys.exit(1)
    ...
    clean_lines = [line for line in lines if not _is_in_section('[search]', line, lines)]
    ...
    if not search_config:
        logger.info("No [search] section found or it was empty. Search command will be disabled.")
...
def _load_and_validate_paths(config: configparser.ConfigParser) -> Dict[str, str]:
    ...
    movies_path = os.path.expanduser(movies_path_str.strip()) if movies_path_str else default_path

    tv_shows_path_str = config.get('host', 'tv_shows_save_path', fallback=None)
    tv_shows_path = os.path.expanduser(tv_shows_path_str.strip()) if tv_shows_path_str else default_path
