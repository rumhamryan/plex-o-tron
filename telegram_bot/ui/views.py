...
from telegram_bot.services.media_manager import get_dominant_file_type, parse_resolution_from_name
...
async def send_confirmation_prompt(
    ...
    source_value = context.user_data.pop('pending_magnet_link', None) or context.user_data.get('torrent_file_path')

    if not source_value:
        logger.error("Could not determine source for pending torrent. Aborting confirmation.")
        await safe_edit_message(progress_message, "❌ An internal error occurred. Could not prepare the download.")
        return
