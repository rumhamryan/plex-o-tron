...
def restart_plex_endpoint():
    ...
    if system == "Linux":
        ...
        command_to_run = [
            "powershell",
            "-Command",
            "Restart-Service -Name 'Plex Media Server'",
        ]
    ...
    try:
        ...
        return (
            jsonify({"status": "success", "message": "Plex restart command sent."}),
            200,
        )
        ...
        return (
            jsonify({"status": "error", "message": f"Command failed: {error_output}"}),
            500,
        )
    except FileNotFoundError:
        print(
            "[AGENT] Error: 'sudo' or the command itself not found. Ensure the command is correct and the agent has the right permissions."
        )
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "'sudo' or 'powershell' not found. Check agent permissions.",
                }
            ),
            500,
        )
...
if __name__ == "__main__":
    print("--- Plex Restart Agent ---")
    print(f"Listening for requests on port {PORT}")
    print(
        "IMPORTANT: Ensure you have configured any firewalls to allow incoming traffic on this port."
    )
