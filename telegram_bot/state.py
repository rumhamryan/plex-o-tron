...
def save_state(file_path: str, active_downloads: Dict, download_queues: Dict) -> None:
    ...
    try:
        ...
        logger.info(f"Saved state: {len(serializable_active)} active, {queued_count} queued downloads.")
...
def load_state(file_path: str) -> Tuple[Dict, Dict]:
    ...
    if not os.path.exists(file_path):
        logger.info(f"Persistence file '{file_path}' not found. Starting with a fresh state.")
        return {}, {}
    ...
    try:
        with open(file_path, 'r') as f:
            ...
            logger.info(f"Loaded state: {len(active)} active, {queued_count} queued downloads.")
            return active, queued
    except (json.JSONDecodeError, IOError) as e:
        logger.error(f"Could not read or parse persistence file '{file_path}': {e}. Starting fresh.")
        return {}, {}
...
async def post_init(application: Application) -> None:
    ...
    """
    from .services.download_manager import download_task_wrapper # Avoid circular import
