...
def build_content(args: argparse.Namespace) -> str:
    ...
    if not example_term:
        ...
        try:
            ...
            default_term = ""
        example_term = _prompt("What was the search term used in that example URL?", default_term)

    derived_base, derived_path = _derive_template_from_example(example_url, example_term)
    if derived_base:
    ...
    movie_category = args.movie_category or (detection and detection.get("movie_cat")) or "Movies"
    ...
    name_selector = args.name_selector or (detection and detection.get("name")) or "td a"
    magnet_selector = (
        args.magnet_selector
    ...
    seeders_selector = (
        args.seeders_selector or (detection and detection.get("seeders")) or "td:nth-of-type(5)"
    )
    leechers_selector = (
        args.leechers_selector or (detection and detection.get("leechers")) or "td:nth-of-type(6)"
    )
    size_selector = (
        args.size_selector or (detection and detection.get("size")) or "td:nth-of-type(4)"
    )
    uploader_selector = (
    ...
    if ("eztv" in site_lc) or ("eztv" in base_lc) or ("eztvx.to" in base_lc):
        ...
        magnet_selector = "a.epinfo"  # detail page; magnets resolved via details selectors
...
def main() -> None:
    ...
    parser.add_argument("--name", dest="name_flag", help="Alias for --name (backward compat)")
    ...
    parser.add_argument("--example-url", help="Example search results URL from the site")
    parser.add_argument(
        "--example-query",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Output path. Defaults to telegram_bot/scrapers/configs/<site>.yaml based on --name."
        ),
    )
    parser.add_argument(
        "--media",
...
def autodetect_from_site(base_url: str) -> dict[str, Any]:
    ...
    for tpl in candidates:
        url = urlparse.urljoin(base_url, tpl.format(query=urlparse.quote_plus(seed_query)))
        html = _get(url)
        if not html:
            continue
...
def _infer_columns(table: Tag) -> dict[str, str]:
    ...
    for i, td in enumerate(tds, start=1):
        ...
        if ({"leeches", "leech", "leechers"} & classes or not has_leeches_td) and leech_td is None:
            leech_td = td if ({"leeches", "leech", "leechers"} & classes) else leech_td
            leech_idx = leech_idx or (i if ({"leeches", "leech", "leechers"} & classes) else 0)
    ...
    seeders_selector = "td.seeds" if has_seeds_td or seeds_td is not None else nth(seeds_idx)
    leechers_selector = "td.leeches" if has_leeches_td or leech_td is not None else nth(leech_idx)
    ...
    if isinstance(name_td, Tag) or has_name_td:
        ...
        name_classes = _get_attr_list(use_td, "class") if isinstance(use_td, Tag) else []
        td_sel = (
            "td.name"
...
def _upgrade_template_with_placeholders(tpl: str) -> str:
    ...
    if m:
        repl = "{query}/{category}/{page}/" if had_trailing else "{query}/{category}/{page}"
...
def _derive_template_from_example(example_url: str, example_term: str) -> tuple[str, str]:
    ...
    if query:
        ...
        try:
            pairs = urlparse.parse_qsl(query, keep_blank_values=True)
            out_parts = [f"{urlparse.quote_plus(k)}={urlparse.quote_plus(v)}" for k, v in pairs]
...
def _update_config_ini_with_site(
    ...
    *,
    ...
    if search_start is None:
        raise RuntimeError("[config.ini] Could not locate [search] section after insertion.")
    ...
    if websites_line is None:
        insert_at = next_key_or_section if next_key_or_section is not None else len(lines)
    ...
    def build_url(cat: str) -> str:
        try:
            ...
            return urlparse.urljoin(base_url + "/", search_path.replace("{query}", "{query}"))
