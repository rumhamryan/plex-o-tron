...
def load_site_config(config_path: Path) -> dict[str, Any]:
    ...
    from .scrapers import adapters as scraper_adapters

    raw_config = scraper_adapters.read_text(resolved_path)
...
class GenericTorrentScraper:
    ...
    def __init__(self, site_config: dict[str, Any]) -> None:
        ...
        self.details_selectors: dict[str, Any] = site_config.get("details_page_selectors", {})
        self.advanced_features: dict[str, Any] = site_config.get("advanced_features", {})
        ...
        if not hasattr(fuzz, scorer_name):
            logger.warning("[SCRAPER] Unknown fuzz scorer '%s'; defaulting to 'ratio'", scorer_name)
    ...
    async def search(
        ...
        if not category_path:
            logger.error(f"[SCRAPER] Media type '{media_type}' not mapped for {self.site_name}")
            return []

        formatted_query = urllib.parse.quote_plus(query)
        search_path = self.search_path.format(query=formatted_query, category=category_path, page=1)
        ...
        for base_url in self._iter_base_url_candidates():
            ...
            logger.info(f"[SCRAPER] {self.site_name}: Fetching search results from {search_url}")
        ...
        target_season = (
            target_details.get("season") if isinstance(target_details.get("season"), int) else None
        )
        target_episode = (
        ...
        def _matches_target_episode(parsed_info: dict[str, Any], raw_title: str) -> bool:
        ...
        def _tokens_no_stop_local(text: str) -> set[str]:
            return {tok for tok in _normalize_local(text).split() if tok and tok not in LOCAL_STOP}
        ...
        if not strong_candidates:
            ...
            logger.info(f"[SCRAPER] {self.site_name}: No strong candidates for '{query}'")
            return []
        ...
        if not base_name_counts:
            logger.info(f"[SCRAPER] {self.site_name}: Unable to determine consensus for '{query}'")
            return []
        ...
        try:
            ...
            def _tokens_no_stop(text: str) -> set[str]:
                return {tok for tok in _normalize(text).split() if tok and tok not in STOP_WORDS}
            ...
            if (
                ...
                if candidates_ok:
                    ...
                    new_best = max(candidates_ok, key=lambda b: (_tier(b), base_name_counts[b]))
    ...
    def _extract_data_from_row(self, row: Tag) -> Optional[TorrentData]:
        ...
        magnet_url = link if isinstance(link, str) and link.startswith("magnet:") else None
    ...
    def _log_candidate_filter_metrics(
        ...
        *,
        ...
        if len(metrics) > max_entries:
            lines.append(f"... {len(metrics) - max_entries} additional candidates omitted ...")
        lines.append("--------------------")
        logger.info("\n".join(lines))

    async def _fetch_page(self, url: str, *, referer_base_url: str | None = None) -> str | None:
        ...
        try:
            response = await scraper_adapters.fetch_page(
                url, headers=headers, timeout=30, follow_redirects=True
            )
            logger.debug(f"[SCRAPER] {self.site_name}: GET {url} -> {response.status_code}")
            response.raise_for_status()
            logger.debug(f"[SCRAPER] {self.site_name}: Response snippet: {response.text[:200]!r}")
