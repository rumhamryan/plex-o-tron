...
from ..config import ALLOWED_EXTENSIONS, logger, PERSISTENCE_FILE 
    ...
    async def report(self, status: lt.torrent_status) -> None: #type: ignore
        ...
        async with self.download_data['lock']:
            ...
            if self.parsed_info.get('type') == 'tv':
                ...
                episode_details_str = escape_markdown(f'S{s:02d}E{e:02d} - {ep_title}', version=2)
...
async def download_task_wrapper(download_data: Dict, application: Application):
    ...
    reporter = ProgressReporter(
        application, chat_id, message_id,
    ...
    try:
        ...
        logger.error(f"Unexpected exception in download task for '{clean_name}': {e}", exc_info=True)
        ...
        if download_data.get('requeued'):
            ...
            await _finalize_download(application, chat_id, message_id, message_text, source_dict, initial_save_path)
...
async def _finalize_download(application, chat_id, message_id, message_text, source_dict, save_path):
    ...
    cleanup_download_resources(application, chat_id, source_dict['type'], source_dict['value'], save_path)


def cleanup_download_resources(application: Application, chat_id: int, source_type: str, source_value: str, base_save_path: str):
...
async def add_download_to_queue(update, context):
    ...
    if not pending_torrent:
        await safe_edit_message(query.message, text="This action has expired. Please send the link again.")
        return
...
async def handle_pause_request(update, context):
    ...
    if chat_id_str in active_downloads:
        ...
        await safe_edit_message(query.message, text="ℹ️ Could not find an active download to pause.")
...
async def handle_resume_request(update, context):
    ...
    if chat_id_str in active_downloads:
        ...
        await safe_edit_message(query.message, text="ℹ️ This download is in the queue and will resume automatically.")
...
async def handle_cancel_request(update, context):
    ...
    if chat_id_str not in active_downloads:
        await safe_edit_message(query.message, text="ℹ️ Could not find an active download to cancel.")
        return
