...
_TRACKER_QUERY = "".join(f"&tr={urllib.parse.quote_plus(tracker)}" for tracker in _TPB_TRACKERS)
...
async def scrape_tpb(
    ...
    **kwargs: Any,
    ...
    media_type_key = (
        "tv" if isinstance(media_type, str) and media_type.lower().startswith("tv") else "movie"
    )
    ...
    if not preferences:
        logger.info("[SCRAPER] TPB: Preferences not configured for %s searches.", prefs_key)
        return []
    ...
    try:
        limit_value = int(limit) if isinstance(limit, int) and limit > 0 else _DEFAULT_LIMIT
    except Exception:
    ...
    year_enriched_base_filter = (
        f"{parsed_base_title} {parsed_base_year}" if parsed_base_year else parsed_base_title
    ).strip()
...
def _transform_results(
    ...
    *,
    ...
    normalized_target = _normalize(f"{target_title} {target_year}" if target_year else target_title)
    ...
    for entry in entries:
        ...
        info_url = f"https://thepiratebay.org/description.php?id={entry_id}" if entry_id else None
...
def _build_magnet(info_hash: str, name: str) -> str:
    return f"magnet:?xt=urn:btih:{info_hash}&dn={urllib.parse.quote_plus(name)}{_TRACKER_QUERY}"
...
def _matches_tv(
    ...
    if parsed_season == target_season or (parsed_is_pack and parsed_season == target_season):
        return True
    return _title_contains_tokens(raw_title, tokens)
