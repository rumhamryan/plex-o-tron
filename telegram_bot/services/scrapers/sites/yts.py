...
async def scrape_yts(
    ...
    **kwargs,
    ...
    try:
        async with adapters.create_async_client(timeout=30, follow_redirects=True) as client:
            ...
            def _passes_gate(title_text: str) -> bool:
                ...
                return any(t in cand_tokens for t in base_tokens) if base_tokens else True
            ...
            async def _api_fallback() -> list[dict[str, Any]]:
                        ...
                        try:
                            ...
                            for tor in mv.get("torrents", []) or []:
                                ...
                                if not info_hash:
                                    continue
                                title_full = (
                                    f"{mv_title} [{tor.get('quality')}.{tor.get('type')}] [YTS.MX]"
                                )
                                trackers = "&tr=" + "&tr=".join(
                                    [
                ...
                async def _call_list_movies(
                    ...
                    try:
                        logger.debug(f"[SCRAPER] YTS API call: {url} with params {params}")
                        resp = await client.get(url, params=params)
                        resp.raise_for_status()
                        payload = resp.json()
                    except Exception as exc:  # noqa: BLE001
                        logger.debug("[SCRAPER] YTS API fallback request failed: %s", exc)
                        return []
                    ...
                    if not isinstance(movies_raw, list):
                        logger.debug("[SCRAPER] YTS API response missing 'movies' list.")
                        return []
                    ...
                    if len(movies_raw) > 0 and len(movies_raw) <= 5:
                        sample_titles = [(m.get("title"), m.get("year")) for m in movies_raw]
                ...
                params_no_quality = {k: v for k, v in base_params.items() if k != "quality"}
            ...
            if not choices:
                if year_str:
                    ...
                    logger.warning(f"[SCRAPER] YTS Stage 1: No movies found for '{query}'.")
                    return []
            ...
            is_confident = bool(
                best_match and len(best_match) == 3 and best_match[1] >= (80 if year_str else 86)
            )
            gated_ok = True
            ...
            for attempt in range(1, 4):
                ...
                try:
                    ...
                    logger.debug(
                        ("[SCRAPER] YTS API attempt %s request error: %s. Retrying in %ss."),
                        attempt,
                        e,
                        delay,
                    )
                ...
                _data_field = api_data.get("data") if isinstance(api_data, dict) else None
                movie_data = _data_field.get("movie") if isinstance(_data_field, dict) else None
                torrents = movie_data.get("torrents", []) if isinstance(movie_data, dict) else []
                conditions = []
                ...
                logger.debug(
                    ("[SCRAPER] YTS API attempt %s succeeded in %.2fs with %d torrents."),
                    attempt,
                    duration,
                    len(torrents),
                )
            ...
            movie_title = (
                movie_data.get("title_long", query) if isinstance(movie_data, dict) else query
            )
            ...
            for torrent in torrents:
                ...
                if not resolution or (
                    ...
                    full_title = (
                        f"{movie_title} [{torrent.get('quality')}.{torrent.get('type')}] [YTS.MX]"
                    )
