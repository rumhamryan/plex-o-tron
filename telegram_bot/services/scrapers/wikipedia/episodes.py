...
from .cache import _WIKI_TITLES_CACHE
from .dates import _extract_release_date_iso
from .fetch import _fetch_html_from_page
from .normalize import _YEAR_HEADER_TOKENS, _sanitize_wikipedia_title
...
async def fetch_episode_title_from_wikipedia(
    ...
    try:
        ...
        if not _last_resort:
            ...
            logger.info(f"[WIKI] Retrying with TV qualifier as last resort: '{qualified}'")
    ...
    if not episode_title:
        logger.warning(f"[WIKI] All parsing strategies failed to find S{season:02d}E{episode:02d}.")
        if not _last_resort:
            qualified = f"{show_title} (TV series)"
            logger.info(f"[WIKI] Retrying with TV qualifier as last resort: '{qualified}'")
            return await fetch_episode_title_from_wikipedia(
...
async def fetch_episode_titles_for_season(
    ...
    cached = _WIKI_TITLES_CACHE.get(cache_key)
    if cached:
        logger.info(f"[WIKI] Cache hit for episode titles: '{show_title}' S{season:02d}.")
    ...
    try:
        ...
        if main_page.title != show_title:
            ...
            logger.info(f"[WIKI] Title corrected: '{show_title}' -> '{canonical_title}'.")
    ...
    try:
        ...
        logger.info(f"[WIKI] Attempting dedicated list page lookup: '{direct_query_user}'.")
    ...
    if not html_to_scrape:
        logger.warning(f"[WIKI] No HTML retrieved for '{canonical_title}'. Returning empty titles.")
        if not _last_resort:
            qualified = f"{show_title} (TV series)"
            logger.info(f"[WIKI] Retrying with TV qualifier as last resort: '{qualified}'")
            return await fetch_episode_titles_for_season(qualified, season, _last_resort=True)
...
async def fetch_total_seasons_from_wikipedia(
    ...
    try:
        ...
        logger.debug(f"[WIKI] Failed parsing 'Series overview' table for '{canonical_title}': {e}")
    ...
    try:
        ...
        if seasons_found:
            ...
            logger.info(f"[WIKI] Season count for '{canonical_title}' (from headers): {count}")
            return count
    except Exception as e:
        logger.debug(f"[WIKI] Failed parsing season headers for '{canonical_title}': {e}")
...
async def _parse_episode_tables(soup: BeautifulSoup, season: int, episode: int) -> str | None:
    ...
    header_tag = soup.find(
        lambda tag: tag.name in ["h2", "h3"] and bool(season_header_pattern.search(tag.get_text()))
    )
    if isinstance(header_tag, Tag):
        ...
        if isinstance(target_table, Tag):
            ...
            return await _extract_title_from_dedicated_table(target_table, season, episode)
    ...
    if isinstance(episodes_header_tag, Tag):
        ...
        if isinstance(target_table, Tag):
            ...
            return await _extract_title_from_embedded_table(target_table, season, episode)

    logger.warning(f"[WIKI] All parsing strategies failed to find S{season:02d}E{episode:02d}.")
    return None


async def _extract_titles_for_season(soup: BeautifulSoup, season: int) -> dict[int, dict[str, Any]]:
...
async def _extract_title_from_dedicated_table(table: Tag, season: int, episode: int) -> str | None:
    for row in table.find_all("tr")[1:]:
        if not isinstance(row, Tag):
            continue
...
async def _extract_title_from_embedded_table(table: Tag, season: int, episode: int) -> str | None:
    for row in table.find_all("tr")[1:]:
        if not isinstance(row, Tag):
            continue
...
async def fetch_season_episode_count_from_wikipedia(
    ...
    logger.info(f"[WIKI] Fetching episode count for '{show_title}' S{season:02d} from Wikipedia.")
    html_to_scrape = None
    try:
        logger.debug(
        ...
        try:
            ...
            if not search_results:
                logger.error(f"[WIKI] No search results for '{show_title}' during fallback.")
                return None
            ...
            logger.debug(f"[WIKI] Disambiguation for '{show_title}'. Options: {options_preview}")
            try:
                choice = e.options[0]
                ...
                logger.error(f"[WIKI] Failed to resolve disambiguation for '{show_title}': {e2}")
                return None
        except Exception as e:
            try:
                ...
                logger.debug(f"[WIKI] Autosuggest diagnostic raised: {type(diag).__name__}: {diag}")
    ...
    if not isinstance(header_row, Tag):
        logger.debug(f"[WIKI] Header row not found in overview table for '{show_title}'.")
        return None
    ...
    for idx, text in enumerate(header_cells):
        if "last" in text and "air" in text:
            ...
            break
    for idx, text in enumerate(header_cells):
        if ("originally" in text and "air" in text) or ("original" in text and "release" in text):
    ...
    for row in overview_table.find_all("tr")[1:]:
        ...
        if season_num == season:
            ...
            last_text = _cell_text(last_aired_col_index) if last_aired_col_index != -1 else ""
            orig_text = (
                _cell_text(originally_aired_col_index) if originally_aired_col_index != -1 else ""
            )
            ...
            if isinstance(count_from_titles, int) and count_from_titles > 0 and ep_count:
                logger.info(
