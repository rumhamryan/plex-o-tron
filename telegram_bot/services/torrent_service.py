...
async def _fetch_and_parse_magnet_details(
    magnet_links: List[str], context: ContextTypes.DEFAULT_TYPE, progress_message: Message
) -> List[Dict[str, Any]]:
    ...
    async def fetch_one(magnet_link: str, index: int):
        ...
        if bencoded_metadata:
            ...
            return {
                "index": index, "ti": ti, "magnet_link": magnet_link,
...
async def fetch_metadata_from_magnet(
    ...
    if context.bot_data is None:
        logger.error("fetch_metadata_from_magnet: context.bot_data is None. Cannot access torrent session.")
        return None
...
async def _update_fetch_timer(progress_message: Message, timeout: int, cancel_event: asyncio.Event):
    ...
    while not cancel_event.is_set():
        ...
        await safe_edit_message(progress_message, text=message_text, parse_mode=ParseMode.MARKDOWN_V2)
