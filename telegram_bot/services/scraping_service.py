...
async def fetch_episode_title_from_wikipedia(
    ...
    try:
        ...
        logger.info(f"[WIKI] Attempting to find dedicated episode page: '{direct_query}'")
        page = await asyncio.to_thread(wikipedia.page, direct_query, auto_suggest=False, redirect=True)
        ...
        logger.warning(f"[WIKI] No dedicated page found for '{show_title}'. Falling back to main show page.")
        try:
            page = await asyncio.to_thread(wikipedia.page, show_title, auto_suggest=True, redirect=True)
            html_to_scrape = await asyncio.to_thread(page.html)

            # Check if Wikipedia corrected the title for us
            if page.title != show_title:
                corrected_show_title = page.title
                logger.info(f"[WIKI] Title was corrected: '{show_title}' -> '{corrected_show_title}'")
            else:
                logger.info(f"[WIKI] Successfully found main show page with original title.")

        except Exception as e:
            logger.error(f"[WIKI] An unexpected error occurred during fallback page search: {e}")
            return None, None

    except Exception as e:
        logger.error(f"[WIKI] An unexpected error occurred during direct Wikipedia search: {e}")
        return None, None
    ...
    if not episode_title:
        logger.warning(f"[WIKI] Both parsing strategies failed to find S{season:02d}E{episode:02d}.")
...
async def _parse_dedicated_episode_page(soup: BeautifulSoup, season: int, episode: int) -> Optional[str]:
    ...
    if not isinstance(index_table, Tag):
        logger.info("[WIKI] Could not find 'Series overview' table. Aborting Primary Strategy.")
        return None
    ...
    for row in index_table.find_all('tr')[1:]:
        if isinstance(row, Tag) and (cells := row.find_all(['th', 'td'])):
            ...
            if season_num_from_cell == season:
                ...
                logger.info(f"[WIKI] Match for Season {season} found. Calculated target table index: {target_table_index}")
                break
        current_table_index_counter += 1

    if target_table_index == -1 or target_table_index >= len(all_tables):
        logger.warning(f"[WIKI] Could not find Season {season} in the index table or index is out of bounds.")
        return None
    ...
    for row in target_table.find_all('tr')[1:]:
        ...
        try:
            ...
            if episode_num_from_cell == episode:
                ...
                logger.info(f"[SUCCESS] Found title via Primary Strategy: '{cleaned_title}'")
                return cleaned_title
        except (ValueError, IndexError):
            continue
            
    logger.warning("[WIKI] Primary Strategy failed to find the episode in the correct table.")
    return None
    ...
    for table in tables:
        ...
        for row in table.find_all('tr')[1:]: # Skip header
            ...
            try:
                ...
                match_found = bool(re.search(fr'\b{season}\b.*\b{episode}\b', row_text_for_match))
                ...
                if match_found:
                    ...
                    logger.info(f"[SUCCESS] Found title via Fallback Strategy: '{cleaned_title}'")
                    return cleaned_title
            except (ValueError, IndexError):
                continue
...
async def scrape_1337x(
    ...
    if not preferences:
        logger.warning(f"[SCRAPER] No preferences found for '{prefs_key}'. Cannot score 1337x results.")
        return []
        ...
        async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
            ...
            for movie_wrapper in soup.find_all('div', class_='browse-movie-wrap'):
                ...
                if isinstance(title_tag, Tag):
                    if (href := title_tag.get('href')) and (title_text := title_tag.get_text(strip=True)):
                        choices[href] = title_text
            
            if not choices:
                logger.warning(f"[SCRAPER] YTS Stage 1: No movies found matching year '{year}'.")
                return []
            ...
            if not isinstance(best_page_url, str):
                logger.error(f"[SCRAPER ERROR] YTS Stage 1: Matched item key was not a string URL. Got: {best_page_url}")
                return []
            ...
            if not (isinstance(movie_info_div, Tag) and (movie_id := movie_info_div.get('data-movie-id'))):
                logger.error(f"[SCRAPER ERROR] YTS Stage 2: Could not find data-movie-id on page {best_page_url}")
                return []
            ...
            if api_data.get('status') != 'ok' or 'movie' not in api_data.get('data', {}):
                logger.error(f"[SCRAPER ERROR] YTS API returned an error: {api_data.get('status_message')}")
                return []
