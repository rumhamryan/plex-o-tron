...
async def test_download_with_progress_http_status_error(mocker):
    ...
    mocker.patch(
        "telegram_bot.services.download_manager.download_core.fetch_url",
...
async def test_download_with_progress_reports_during_pause(mocker):
    ...
    download_data = {"lock": asyncio.Lock(), "is_paused": False}
    mocker.patch(
        "telegram_bot.services.download_manager.download_core.lt.parse_magnet_uri",
...
@pytest.mark.asyncio
async def test_add_season_to_queue(mocker, make_update, make_callback_query, make_message, context):
    ...
    context.user_data["pending_season_download"] = [
        {
            "link": "magnet1",
            "parsed_info": {"title": "Example Show", "season": 5, "episode": 1},
        },
        {
            "link": "magnet2",
            "parsed_info": {"title": "Example Show", "season": "5", "episode": 2},
        },
    ]
    context.bot_data["active_downloads"] = {}
    context.bot_data["download_queues"] = {}
...
async def test_cancel_request_flag_flow(
    ...
    update_start = make_update(callback_query=make_callback_query("cancel_download", message))
    await handle_cancel_request(update_start, context)
    assert download_data.get("cancellation_pending") is True

    # Deny cancellation
    update_deny = make_update(callback_query=make_callback_query("cancel_deny", message))
    ...
    await handle_cancel_request(update_start, context)
    assert download_data.get("cancellation_pending") is True
    update_confirm = make_update(callback_query=make_callback_query("cancel_confirm", message))
