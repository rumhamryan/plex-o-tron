...
from telegram_bot.services.scrapers import YtsScraper, wikipedia_scraper
...
async def test_dry_run_flow_uses_wiki_year_and_filters_resolution(mocker):
    ...
    # Check if query is in args or kwargs (it's passed as positional in orchestrate_searches)
    assert (
        base_for_search in yts_call.args
        or yts_call.kwargs.get("query") == base_for_search
    )
    assert (
        f"{base_for_search} 1979" in x_call.args
        or x_call.kwargs.get("query") == f"{base_for_search} 1979"
    )
...
async def test_dry_run_movie_explicit_year_overrides_wiki(mocker):
    ...
    assert (
        f"{base_for_search} 1982" in x_call.args
        or x_call.kwargs.get("query") == f"{base_for_search} 1982"
    )
