...
from telegram_bot.services.scrapers import (
...
async def main():
    ...
    while True:
        try:
            ...
            if mode == "tv":
                show_title = input("Enter show title (e.g., 'The Office (American TV series)'): ")
                ...
                (
                    episode_title,
                    corrected_title,
                ) = await fetch_episode_title_from_wikipedia(show_title, season, episode)
                ...
                years, corrected_title = await fetch_movie_years_from_wikipedia(movie_title)
