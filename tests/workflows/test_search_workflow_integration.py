...
from telegram_bot.workflows.search_workflow import tv_flow
...
async def test_tv_season_fallback_uses_wiki_titles_and_corrected_title(mocker):
    ...
    mocker.patch(
        "telegram_bot.workflows.search_workflow.tv_flow.parse_torrent_name",
        return_value={},
    )
    ...
    await tv_flow._perform_tv_season_search(
        message,
        ctx,
        title="Show",
        season=1,
