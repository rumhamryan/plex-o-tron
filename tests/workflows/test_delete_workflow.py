...
async def test_delete_show_happy_path(mocker, context, make_callback_query, make_message):
    ...
    mocker.patch("telegram_bot.workflows.delete_workflow.os.path.exists", return_value=True)
    mocker.patch("telegram_bot.workflows.delete_workflow.os.path.isdir", return_value=True)
    mocker.patch(
    ...
    await handle_delete_buttons(Update(update_id=1, callback_query=make_callback_query("delete_start_tv", make_message())), context)
    ...
    await handle_delete_buttons(Update(update_id=3, callback_query=make_callback_query("delete_tv_all", make_message())), context)
    assert context.user_data["path_to_delete"] == "/path/show"

    # Confirm deletion
    context.bot_data["PLEX_CONFIG"] = {"url": "u", "token": "t"}
    await handle_delete_buttons(Update(update_id=4, callback_query=make_callback_query("confirm_delete", make_message())), context)
    delete_mock.assert_awaited_once_with("/path/show", {"url": "u", "token": "t"}, context)
