...
@pytest.mark.asyncio
async def test_delete_show_happy_path(mocker, context, make_callback_query, make_message):
    mocker.patch(
    ...
    rmtree_mock = mocker.patch("telegram_bot.workflows.delete_workflow.filesystem.shutil.rmtree")
    mocker.patch(
        "telegram_bot.workflows.delete_workflow.filesystem.os.path.exists", return_value=True
    )
    mocker.patch(
        "telegram_bot.workflows.delete_workflow.filesystem.os.path.isdir", return_value=True
    )
    mocker.patch(
        "telegram_bot.workflows.delete_workflow.filesystem.asyncio.to_thread",
    ...
    await handle_delete_workflow(Update(update_id=2, message=make_message("My Show")), context)
...
@pytest.mark.asyncio
async def test_confirm_delete_skips_plex_when_item_has_multiple_parts(
    mocker, context, make_callback_query, make_message, tmp_path
):
    safe_edit_mock = mocker.patch(
        "telegram_bot.workflows.delete_workflow.handlers.safe_edit_message",
        new=AsyncMock(),
    ...
    mocker.patch(
        "telegram_bot.workflows.delete_workflow.plex.PlexServer",
        return_value=mocker.Mock(),
    )

    root = tmp_path / "movies"
    root.mkdir()
    selected = root / "Inception (2010).mp4"
    selected.write_text("data")
    alternate_dir = root / "alternate"
    alternate_dir.mkdir()
    alternate = alternate_dir / "Inception (2010).mkv"
    alternate.write_text("more data")

    part_one = mocker.Mock(file=str(selected))
    part_two = mocker.Mock(file=str(alternate))
    media = mocker.Mock(parts=[part_one, part_two])
    plex_item = mocker.Mock(title="Inception", media=[media])
    mocker.patch(
        "telegram_bot.workflows.delete_workflow.plex._find_media_in_plex_by_path",
        return_value=plex_item,
    )

    context.user_data["path_to_delete"] = str(selected)
    context.user_data["delete_target_kind"] = "movie_file"
    context.bot_data["PLEX_CONFIG"] = {"url": "u", "token": "t"}
    ...
    assert alternate.exists() is True
    plex_item.delete.assert_not_called()
    final_text = safe_edit_mock.await_args.kwargs["text"]
    assert "Plex Skipped" in final_text
...
async def test_present_delete_results_lists_show_sizes(mocker, context, make_message):
    safe_edit_mock = mocker.patch(
        "telegram_bot.workflows.delete_workflow.selection.safe_edit_message",
        new=AsyncMock(),
    )
    mocker.patch(
