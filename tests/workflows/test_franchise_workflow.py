...
from telegram_bot.workflows.franchise_workflow import handle_franchise_confirmation, run_franchise_logic
...
async def test_run_franchise_logic_flow():
    ...
    context.bot_data = {"SAVE_PATHS": {"default": "/tmp/downloads", "movies": "/tmp/movies"}}
    ...
    with patch("telegram_bot.services.scraping_service.fetch_franchise_details", new_callable=AsyncMock) as mock_fetch, \
         patch("telegram_bot.services.search_logic.orchestrate_searches", new_callable=AsyncMock) as mock_search, \
         patch("telegram_bot.services.plex_service.create_plex_collection", new_callable=AsyncMock) as mock_plex, \
         ...
         patch("telegram_bot.workflows.franchise_workflow.process_queue_for_user", new_callable=AsyncMock), \
         patch("os.makedirs") as mock_makedirs, \
         patch("os.path.exists", return_value=False):

        mock_fetch.return_value = franchise_info
        mock_search.return_value = [{"title": "The Matrix Reloaded", "size_gb": 5, "page_url": "http://test"}]
