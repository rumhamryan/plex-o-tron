...
# --- Refactored Imports ---
from telegram_bot.config import get_configuration, logger, PERSISTENCE_FILE
...
def register_handlers(application: Application) -> None:
    ...
    application.add_handler(MessageHandler(filters.Regex(re.compile(r'^/?start$', re.IGNORECASE)), help_command)) # /start redirects to help
...
def main() -> None:
    ...
    application = (
        ApplicationBuilder()
        .token(token)
        .post_init(post_init)        # Function to run after initialization (e.g., resume downloads)
